<template>
	<div class="visitStatusWrapper" :class="{ 'small-box': SmallBox }">
		<div
			class="VisitBoxContent"
			:style="`background: ${defaultBoxColor}`"
			:class="{ active: active }"
		>
			<div class="text-center" style="text-align: center;">
				<div
					class="VisitTitle"
					:style="{ paddingTop: progress != null ? '20px' : '30px' }"
				>
					<span v-html="name"></span>
				</div>
				<div
					class="VisitNumber"
					:style="{ top: progress != null ? '-7px' : '6px' }"
				>
					<h1
						class="Number"
						:style="{
						...
						:class="{ 'text-3d': !SmallBox, DummyFontSize: DummyFontSize }"
					>
						{{ completedValue }}
	...
	mounted() {
		...
		this.$root.$on('bv::tooltip::show', (bvEvent) => {
			vm.$nextTick(function() {
				jQuery('.b-tooltip-' + vm._uid + ' > .tooltip-inner').css(
					'background-color',
					vm.defaultBoxColor
				)
				jQuery('.b-tooltip-' + vm._uid + ' > .arrow').css(
					'border-top-color',
					vm.defaultBoxColor
				)
				jQuery('.b-tooltip-' + vm._uid + ' > .arrow').css(
